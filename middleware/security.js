...
const helmetMiddleware = helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc: ["'self'"],
      connectSrc: ["'self'", "https://api.cloudinary.com", "https://*.vercel.app"],
      frameSrc: ["'none'"],
      objectSrc: ["'none'"]
    }
  },
...
const hppMiddleware = hpp({
  whitelist: [
    'subject', 'topic', 'difficulty', 'page', 'limit', 'sort', 'search'
  ]
});
...
const requestTimeout = (timeoutMs = 30000) => (req, res, next) => {
  req.setTimeout(timeoutMs, () => {
    console.error(`[TIMEOUT] Request timeout on ${req.method} ${req.path} from IP: ${req.ip}`);
    if (!res.headersSent) {
      res.status(408).json({
        success: false,
...
const securityLogger = (req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (res.statusCode >= 400) {
      console.warn(`[${logLevel.toUpperCase()}] ${req.method} ${req.path} - ${res.statusCode} - ${duration}ms - IP: ${req.ip}`);
    }
  });
...
const suspiciousActivityDetector = (req, res, next) => {
  ...
  for (const pattern of suspiciousPatterns) {
    if (pattern.test(combinedInput)) {
      console.error(`[SECURITY ALERT] Suspicious activity detected from IP: ${req.ip}, Path: ${req.path}`);
